...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Tagline
    this._fade(this.add.text(cx,cy+26,'"Your past is your greatest enemy."',{fontFamily:'Share Tech Mono, monospace',fontSize:'13px',color:'#b0c4de',align:'center'}).setOrigin(0.5).setDepth(10).setAlpha(0),900);

    // Controls
    this._fade(this.add.text(cx,cy+56,'WASD/ARROWS — MOVE   SHIFT — WARP   E — POWERUP   ESC — PAUSE',{
      fontFamily:'Share Tech Mono, monospace',fontSize:'10px',color:'#8ea4c2',align:'center'
    ...
    this._diffTexts={};
    const diffY=cy+100;
    this.add.text(cx,diffY,'DIFFICULTY',{fontFamily:'Orbitron, monospace',fontSize:'10px',color:'#8ea4c2'}).setOrigin(0.5).setDepth(10).setAlpha(0);
    this._fade(this.add.text(cx,diffY,'DIFFICULTY',{fontFamily:'Orbitron, monospace',fontSize:'10px',color:'#8ea4c2'}).setOrigin(0.5).setDepth(10).setAlpha(0),1200);
    ...
    diffs.forEach((d,i)=>{
      ...
      const btn=this.add.text(bx,by,lbl,{
        ...
        color:d===this._difficulty?CONFIG.COLOR_CYAN:'#8ea4c2',align:'center'
      }).setOrigin(0.5).setDepth(11).setAlpha(0).setInteractive({useHandCursor:true});
      btn.on('pointerover',()=>{ if(d!==this._difficulty) btn.setColor('#c0d4ec'); });
      btn.on('pointerout', ()=>{ btn.setColor(d===this._difficulty?CONFIG.COLOR_CYAN:'#8ea4c2'); });
    ...
    if(lb.length>0){
      const lbY=cy+158;
      this._fade(this.add.text(cx,lbY,'TOP SCORES',{fontFamily:'Orbitron, monospace',fontSize:'10px',color:'#8ea4c2'}).setOrigin(0.5).setDepth(10).setAlpha(0),1500);
      lb.slice(0,5).forEach((score,i)=>{
        const col=i===0?CONFIG.COLOR_GOLD:i<3?CONFIG.COLOR_CYAN:'#8ea4c2';
  ...
  _setDifficulty(d) {
    ...
    Object.entries(this._diffTexts).forEach(([k,t])=>{
      t.setColor(k===d?CONFIG.COLOR_CYAN:'#8ea4c2');
    });
  }
