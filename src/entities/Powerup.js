...
export class Powerup {
  constructor(scene, type, x, y) {
    this.scene  = scene;
    this.type   = type;   // 'clash' | 'phase'
    this.x      = x;
    this.y      = y;
    this.alive  = true;
    this._phase = Math.random() * Math.PI * 2;
    this._gfx   = scene.add.graphics().setDepth(8);
    this._label = scene.add.text(x, y - 26, type === 'clash' ? '💀 CLASH' : '🛡 PHASE', {
      fontFamily: 'Orbitron, monospace', fontSize: '9px',
      color: type === 'clash' ? CONFIG.COLOR_CLASH : CONFIG.COLOR_PHASE,
      align: 'center'
  ...
  update() {
    ...
    const col   = this.type === 'clash' ? 0xff6600 : 0x00ccff;
    const size  = 11;
    const g     = this._gfx;
    g.clear();
